...
#define ECAP_PRESCALAR_SIZE	5
#define ECAP_PRESCALAR_MASK	GENMASK(ECAP_PRESCALAR_OFFSET + ECAP_PRESCALAR_SIZE - 1, ECAP_PRESCALAR_OFFSET)
#define ECAP_PRESCALAR(flags)	(uint8_t)((flags & ECAP_PRESCALAR_MASK) >> ECAP_PRESCALAR_OFFSET)
...
struct ecap_state {
	...
	struct clk	*clk;
	void __iomem    *regs;
	u32		buf[4] __aligned(8); /* 1x32-bit channel + padding + timestamp */
					...
					char *buf)
{
	...
	res = test_bit(ECAP_POL_CAP1_OFFSET, &state->flags);
					...
					struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&state->lock);
	res = test_bit(ECAP_POL_CAP2_OFFSET, &state->flags);
	mutex_unlock(&state->lock);
					...
					size_t len)
{
	...
	/* Ensure prescalar value is valid. eCAP supports the following
	 * prescalar values: 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
	 * 0x1E, 0x1F.
	 */
	if (val > 0x05 && val != 0x1E && val != 0x1F)
		return -EINVAL;

	state->flags &= ~(0x1F << ECAP_PRESCALAR_OFFSET);
...
int ecap_set_trigger_state(struct iio_trigger *trig, bool enable) {
	struct iio_dev *idev = iio_trigger_get_drvdata(trig);
	struct ecap_state *state = iio_priv(idev);
	int err;

	//dev_dbg(&idev->dev, "TIECAP: Set trigger state: %d", enable);

	return 0;
}

int ecap_trig_try_reenable(struct iio_trigger *trig) {
	struct iio_dev *idev = iio_trigger_get_drvdata(trig);
	struct ecap_state *state = iio_priv(idev);
	u16 ints;

	dev_dbg(&idev->dev, "TIECAP: try reenable");
...
static const struct iio_trigger_ops iio_interrupt_trigger_ops = {
	.set_trigger_state = ecap_set_trigger_state,
	.try_reenable = ecap_trig_try_reenable,
};
				...
				int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		*val = 0;
		*val2 = NSEC_PER_SEC / clk_get_rate(state->clk);
...
static int ecap_probe(struct platform_device *pdev)
{
	...
	idev = devm_iio_device_alloc(&pdev->dev, sizeof(*state));
	if (!idev)
		return -ENOMEM;
